...
function validatePacket(value: unknown, phase: string, path: string, issues: string[]): void {
  ...
  const enumValues: Record<string, string[]> = {
    verdict: phase.includes("review") ? ["PASS", "NEEDS_FIX", "ARCHITECTURAL_ESCALATION"] : phase.includes("architect") ? (phase.includes("final") ? ["ACCEPT", "NEEDS_REMEDIATION"] : ["APPROVE", "CORRECT"]) : ["continue", "stop"],
...
function validateResults(value: unknown, issues: string[]): void {
  ...
  if (!Array.isArray(value.engineers)) issues.push("results.engineers must be an array");
    ...
    else validateOutcome(item.outcome, "execution.engineer", `results.engineers[${index}].outcome`, issues);
  }
  if (!Array.isArray(value.reviewers)) issues.push("results.reviewers must be an array");
    ...
    else validateOutcome(item.outcome, "review.reviewer", `results.reviewers[${index}].outcome`, issues);
  }
...
function validateAttempt(value: unknown, index: number, issues: string[]): void {
  ...
  exactKeys(value, ["attemptId", "role", "phase", "round", "target", "provenance", "recoveryRisk", "preparedAt"], ["spawnRequestedAt", "spawnedAt", "settledAt", "agentId", "packetKind", "packetValidated", "configRevision"], path, issues);
...
function validateCheckpoint(value: unknown, state: Record<string, unknown>, issues: string[]): void {
  ...
  exactKeys(value, ["id", "runId", "revision", "state", "phase", "repairRound", "remediationRounds", "architectEscalations", "leadEscalations"], ["attemptId", "agentId"], "checkpoint", issues);
...
export function validatePersistedFactoryRunState(value: unknown, intendedProjectDir?: string): FactoryStateValidation {
  ...
  exactKeys(state, ["version", "runId", "createdAt", "updatedAt", "task", "cwd", "config", "state", "repairRound", "repairExhausted", "effectiveArchitecture", "remediationRounds", "architectEscalations", "leadEscalations", "results", "metrics", "errors", "parked"], ["inFlight", "waiting", "stoppedReason", "stateRevision", "checkpoint", "attempts", "presetReplacement"], "state", issues);
  if (state.version !== LEGACY_FACTORY_STATE_VERSION && state.version !== 2) issues.push("unsupported Factory state version");
  ...
  validateStateConsistency(state, issues, state.version === 2);
  ...
  if (legacy) {
    if (state.stateRevision !== undefined || state.checkpoint !== undefined || state.attempts !== undefined || state.presetReplacement !== undefined) issues.push("legacy state cannot contain version 2 recovery fields");
  } else {
  ...
  if (state.version === 2 && roles && metrics && integer(metrics.totalAttempts)) {
...
function latestAttempt(state: FactoryRunState, phase: string): FactoryAttempt | undefined {
  return [...(state.attempts ?? [])].reverse().find((attempt) => attempt.phase === phase);
}
...
export function createFactoryCheckpoint(state: FactoryRunState, revision = state.stateRevision ?? 0): FactoryCheckpoint {
  ...
  const parts = [state.runId, state.state, phase, `repair=${state.repairRound}`, `remediation=${state.remediationRounds}`, `architect=${state.architectEscalations}`, `lead=${state.leadEscalations}`, `attempt=${attemptId ?? "none"}`, `agent=${agentId ?? "none"}`, `revision=${revision}`];
  return {
    id: parts.join("|"),
    runId: state.runId,
    revision,
